...
class PostService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        $portalPostModel = $portalPostModel->alias('a')->join('user u', 'a.user_id = u.id');
        ...
        if (!empty($category)) {
            $portalPostModel = $portalPostModel->join('portal_category_post b', 'a.id = b.post_id');
            $field = 'a.*,b.id AS post_category_id,b.list_order,b.category_id,u.user_login,u.user_nickname,u.user_email';
        }

        
        $articles  = $portalPostModel->field($field)
