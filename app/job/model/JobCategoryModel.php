...
class JobCategoryModel extends Model
{
    ...
    public function getJobTotalCountAttr($value, $data)
    {
        $total = JobCategoryPostModel::where('category_id', $data['id'])->where('status', 1)->count();
        return $total;
    }
...
//        }
        ...
        foreach ($categories as $item) {
            ...
            $item['url']            = cmf_url('job/List/index', ['id' => $item['id']]);
            $item['str_action']     = '<a class="btn btn-xs btn-primary" href="' . url("AdminJobCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="btn btn-xs btn-danger js-ajax-delete" href="' . url("AdminJobCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            // $item['str_action']     = '<a class="btn btn-xs btn-primary" href="' . url("AdminJobCategory/add", ["parent" => $item['id']]) . '">添加子分类</a>  <a class="btn btn-xs btn-primary" href="' . url("AdminJobCategory/edit", ["id" => $item['id']]) . '">' . lang('EDIT') . '</a>  <a class="btn btn-xs btn-danger js-ajax-delete" href="' . url("AdminJobCategory/delete", ["id" => $item['id']]) . '">' . lang('DELETE') . '</a> ';
            if ($item['status']) {
                $item['str_action'] .= '<a class="btn btn-xs btn-warning js-ajax-dialog-btn" data-msg="您确定隐藏此分类吗" href="' . url('AdminJobCategory/toggle', ['ids' => $item['id'], 'hide' => 1]) . '">隐藏</a>';
            } else {
                $item['str_action'] .= '<a class="btn btn-xs btn-success js-ajax-dialog-btn" data-msg="您确定显示此分类吗" href="' . url('AdminJobCategory/toggle', ['ids' => $item['id'], 'display' => 1]) . '">显示</a>';
            }
            if ($item['description']) {
    ...
    public function addCategory($data)
    {
        ...
        if ($result != false) {
            ...
            if (!empty($data['alias']) && !empty($id)) {
                $routeModel->setRoute($data['alias'], 'job/List/index', ['id' => $id], 2, 5000);
                $routeModel->setRoute($data['alias'] . '/:id', 'job/Post/index', ['cid' => $id], 2, 4999);
            }
    ...
    public function editCategory($data)
    {
        ...
        if (empty($oldCategory) || empty($newPath)) {
            ...
            if (!empty($categoryAlias)) {
                $routeModel->setRoute($categoryAlias, 'job/List/index', ['id' => $data['id']], 2, 5000);
                $routeModel->setRoute($categoryAlias . '/:id', 'job/Post/index', ['cid' => $data['id']], 2, 4999);
            } else {
                $routeModel->deleteRoute('job/List/index', ['id' => $data['id']]);
                $routeModel->deleteRoute('job/Post/index', ['cid' => $data['id']]);
            }
