...
// +----------------------------------------------------------------------
namespace plugins\hz_back_sql\controller;

use cmf\controller\PluginAdminBaseController;
use plugins\hz_back_sql\service\Dir;
...
class AdminIndexController extends PluginAdminBaseController
{
    ...
    public function index()
    {
        ...
        $this->assign('tables', Db::getTables());  // 显示所有数据表
        return $this->fetch();
    }
    ...
    public function index_post()
    {
        ...
        if ($this->request->isPost() || isset($_GET['dosubmit'])) {
            ...
            if (isset($_GET['type']) && $_GET['type'] == 'url') {
                ...
                if (is_dir(CMF_ROOT . $this->backup_path . $this->backup_name)) {
                    $this->error('备份名称已经存在'.CMF_ROOT . $this->backup_path . $this->backup_name);
                }
                mkdir(CMF_ROOT . $this->backup_path . $this->backup_name,0777,true);

                if (!is_file(CMF_ROOT . $this->backup_path . $this->backup_name .
                    '/tbl_queue.log')) {
                    //写入队列
            ...
            return $this->_deal_result($tables, $vol, $sizelimit);
        }
    }
    ...
    public function import()
    {
        ...
        if ($this->_import_vol($backup['file'])) {
            if ($vol < count($backups)) {
                $vol++;
                $link = cmf_plugin_url("HzBackSql://admin_index/import",array("vol"=>$vol,"backup"=>urlencode($this->backup_name)));
                $this->success(sprintf('导入成功！', $vol - 1), $link);
            } else {
                $this->success('导入成功！', cmf_plugin_url("HzBackSql://admin_index/restore"));
            }
        } else {
            $this->error('请直接下载备份文件进行备份！');
        }
    }

    private function _import_vol($sql_file_name)
    {
        $sql_file = CMF_ROOT . $this->backup_path . $this->backup_name . '/' . $sql_file_name;
    ...
    public function del_backup()
    {
        ...
        $dir = new Dir();
        $dir->delDir(CMF_ROOT . $this->backup_path . $_GET['backup'].'/');
        $this->success('操作成功！');
    }
    ...
    public function download()
    {
        ...
        $backup_name = isset($get['backup']) && trim($get['backup']) ? trim($get['backup']) :
            $this->error('请选择备份名称！');
        $file = isset($get['file']) && trim($get['file']) ? trim($get['file']) : $this->
        error('请选择备份文件！');
        $sql_file = CMF_ROOT . $this->backup_path . $backup_name . '/' . $file;
    ...
    private function _get_vols($backup_name)
    {
        $vols = array(); //所有的卷
        $bytes = 0;
        $vol_path = CMF_ROOT . $this->backup_path . $backup_name . '/';
        if (is_dir($vol_path)) {
    ...
    {
        $backups = array(); //所有的备份
        if (is_dir(CMF_ROOT . $this->backup_path)) {
            if ($handle = opendir(CMF_ROOT . $this->backup_path)) {
                while (($file = readdir($handle)) !== false) {
                    if ($file[0] != '.' && filetype(CMF_ROOT . $this->backup_path . $file) == 'dir') {
                        $backup['name'] = $file;
                        $backup['date'] = filemtime(CMF_ROOT . $this->backup_path . $file) - date('Z');
                        $backup['date_str'] = date('Y-m-d H:i:s', $backup['date']);
                        $backup['vols'] = $this->_get_vols($file);
                        $end_vol = end($backup['vols']);
                        $backup['total_size'] =round($this->_get_dir_size(CMF_ROOT . $this->backup_path . $file)/1024,2);
                        $backups[] = $backup;
                    }
                }
            }
    ...
    private function _deal_result($tables, $vol, $sizelimit)
    {
        ...
        if (empty($tables)) {
            ...
            $this->success('备份成功！',cmf_plugin_url("HzBackSql://admin_index/restore"));
        } else {
            // 开始下一卷
            $this->_set_vol($vol); //设置分卷记录
            $link= cmf_plugin_url("HzBackSql://admin_index/index",array("dosubmit"=>1,"type"=>"url","backup_name"=>$this->backup_name,"sizelimit"=>$sizelimit));
    ...
    private function _get_table_df($table)
    {
        ...
        $tmp_sql  = Db::query("SHOW CREATE TABLE `$table` ");
    ...
    {
        $post_pos = $pos;
        $total = Db::query("SELECT COUNT(*) as count FROM $table"); //数据总数
        $total = $total[0]['count'];
        ...
        for ($i = 0; $i < $cycle_time; $i++) {
            $data = Db::query("SELECT * FROM $table LIMIT " . ($this->
    ...
    {
        $backup_path = CMF_ROOT . '/data/backup/';
    ...
    {
        return file_put_contents(CMF_ROOT . $this->backup_path . $this->backup_name .
    ...
    {
        $tbl_queue_file = CMF_ROOT . $this->backup_path . $this->backup_name .
            '/tbl_queue.log';
    ...
    {
        $log_file = CMF_ROOT . $this->backup_path . $this->backup_name . '/vol.log';
